...
class Constitution:
    ...
    def semantic_check(self, text: str, api_key: str | None = None,
        ...
        Args:
            ...
            base_url: API 地址
            model: 模型名（不传则从 DEEPSEEK_MODEL 环境变量读取，默认 deepseek-chat）
...
示例：VIOLATION:C3 暗示跳过验证流程"""
    ...
    def full_check(self, text: str, api_key: str | None = None,
                   model: str | None = None) -> dict:
        """完整检查：正则 + 语义（双层）。"""
        # 第一层：正则
        ...
        # 第二层：语义（正则全过时才触发）
        semantic = self.semantic_check(text, api_key, model=model)
